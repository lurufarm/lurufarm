/**
 * 전역과 지역 스코프
 * 
 * 전역은 코드의 가장 바깥 영역을 말한다.
 * 전역에 변수를 선언하면 전역 변수가 되며, 어디서든지 참조할 수 있다.
 * 
 * 지역이란 함수 몸체 내부를 말한다.
 * 지역에 변수를 선언하면 지역 변수가 되며, 자신의 지역 스코프와 하위 지역 스코프에서만 유효하다.
 */

let x = 'global x';
let y = 'global y';

function outer()
{
    let z = "outer's local z";
    console.log("=outer=");
    console.log(x);     // global x
    console.log(y);     // global y
    console.log(z);     // outer's local z
    console.log(ㅍ);     // outer's local z

    function inner()
    {
        let x = "inner's local x";
        // let z = "inner's local z";
        let v = "inner's local v";
        console.log("=inner=");
        console.log(x);     // inner's local x
        console.log(y);     // global y
        console.log(z);     // outer's local z -> inner's local z
    }

    inner();
}

outer();

// console.log(x);     // global x
// console.log(z);     // z가 정의되어있지 않기 때문에 참조 에러 발생

/**
 * 모든 스코프는 하나의 계층적 구조로 연결되며, 모든 지역 스코프의 최상위 스코프는 전역 스코프이다.
 * 변수를 참조할 때 자바스크립트 엔진은 스코프 체인을 통해 변수를 참조하는 코드의 스코프에서 시작하여
 * 상위 스코프 방향으로 이동하며 선언된 변수를 검색한다.
 * 따라서 상위 스코프에서 유효한 변수는 하위 스코프에서 자유롭게 참조할 수 있지만
 * 하위 스코프에서 유효한 변수를 상위 스코프에서는 참조 할 수 없다.
 */